...
# 信頼度の低い方を削除する
#
# ★2026-08-25: 積まれた箱同士が隣接して多少重なっても誤って
# ★重複除去(同一物体扱い)されないよう、70%から緩めた。
# ============================================================
...
#
# ★赤モデルの場合
# ★    赤色が15%未満 → 除外(自分の色が足りない)
# ★    青色が15%以上 → 除外(反対色が混入)
#
# ★青モデルの場合
# ★    青色が15%未満 → 除外(自分の色が足りない)
# ★    赤色が15%以上 → 除外(反対色が混入)
#
# ★2026-08-25: 反対色チェックだけだと、自分の色も反対色も
# ★ほぼ無い(背景のみ等)BBOXまで通ってしまうため、
# ★自分の色が最低限あるかのチェックを追加。あわせて
# ★反対色の許容度も30%→15%に厳しくした。
# ============================================================

YOLO_OWN_COLOR_MIN_RATIO = 0.15
YOLO_OPPOSITE_COLOR_RATIO_THRESHOLD = 0.15
...
class BrockMasterNode(Node):
    ...
    def __init__(self):
        ...
        # ★色判定の閾値をログ表示
        self.get_logger().info(
            f"YOLO own color min ratio: "
            f"{YOLO_OWN_COLOR_MIN_RATIO * 100:.0f}%"
        )
    ...
    def _run_inference(self, frame):
        ...
        results = model(
            ...
            imgsz=640,
            iou=0.9,  # 積まれた箱同士の隣接による重なりを誤って統合しないよう緩める(既定0.7)
            verbose=False,
        )
        ...
        for result in results:
            ...
            for box in result.boxes:
                ...
                # ★YOLO BBOX内部の色判定
                #
                # ★自分の色(own)が十分あるか、反対色(opposite)が
                # ★混ざりすぎていないかの両方をチェックする。
                # ★反対色チェックだけだと、自分の色も反対色も
                # ★ほぼ無い(背景等)BBOXまで通ってしまうため、
                # ★自分の色の下限チェックを併用する。
                # =================================================
                ...
                if own_mask is not None:
                    ...
                    own_pixel_count = cv2.countNonZero(
                        own_mask[
                            y1:y2,
                            x1:x2,
                        ]
    ...
    def _draw_boxes(
        ...
        for box_data in box_data_list:
            ...
            # ★YOLO認識の場合は自分の色/反対色割合を表示
            if "opposite_ratio" in box_data:

                label_text += (
                    f" 自:{box_data.get('own_ratio', 0.0) * 100:.1f}%"
                    f" 反:{box_data['opposite_ratio'] * 100:.1f}%"
                )
    ...
    def _build_info(
        ...
        for box_data in box_data_list:
            ...
            # ★YOLO認識の場合は自分の色/反対色割合を送信
            if "opposite_ratio" in box_data:

                info_lines[-1] += (
                    f",own_ratio="
                    f"{box_data.get('own_ratio', 0.0):.3f}"
                    f",opposite_ratio="
